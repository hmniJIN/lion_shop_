package com.likelion.lion_shop.utils;

import com.likelion.lion_shop.Dto.response.JwtDto;
import com.likelion.lion_shop.entity.Token;
import com.likelion.lion_shop.Repository.TokenRepository;
import com.likelion.lion_shop.userDetails.CustomUserDetails;
...
public class JwtUtil {
    ...
    public String tokenProvider(CustomUserDetails customUserDetails, Instant expiration) {
        ...
        return Jwts.builder()
                ...
                .issuedAt(Date.from(issuedAt)) // 현재 시간(발급 시간) 추가
    ...
    public String createJwtAccessToken(CustomUserDetails customUserDetails) {
        Instant expiration = Instant.now().plusMillis(accessExpMs);//엑세스 토큰의 만료 시간을 현재 시간으로부터 accessExpMs 밀리초 후로 설정
        return tokenProvider(customUserDetails, expiration); //tokenProvider 메서드 호출하여 엑세스 토큰 생성 후 반환
    }
    ...
    public String createJwtRefreshToken(CustomUserDetails customUserDetails) {
        ...
        String refreshToken = tokenProvider(customUserDetails, expiration);//tokenProvider 메서드를 호출하여 리프레시 토큰을 생성
    ...
    //throws SignatureException은 JWT 토큰의 서명 검증 과정에서 발생할 수 있는 예외를 처리하기 위한 것
    public JwtDto reissueToken(String refreshToken) throws SignatureException {
        ...
        // 재발급
        //새로운 액세스 토큰과 리프레시 토큰을 생성하여 JwtDto 객체로 묶어서 반환
        return new JwtDto(
